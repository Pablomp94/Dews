...
import es.albares.dwes.paw5jpa.DAO.DireccionDAO;
import es.albares.dwes.paw5jpa.DAO.ProvinciaDAO;
import es.albares.dwes.paw5jpa.DAO.RolDAO;
import es.albares.dwes.paw5jpa.DAO.UsuarioDAO;
import es.albares.dwes.paw5jpa.entidades.Aficion;
import es.albares.dwes.paw5jpa.entidades.Direccion;
import es.albares.dwes.paw5jpa.entidades.Sexo;
        ...
        try {
            ...
            if (aficiones != null) {
                usuario.setAficiones(
                        Stream.of(aficiones).map(Aficion::new).collect(Collectors.toList()));
            }
            ...
            /* comprobamos que las aficiones asignadas desde el formulario coinciden con alguna de
            las aficiones de BD (si no coincide no se inserta en la lista del usuario*/
 /* antes hay que definir el método "equals" en la clase Aficion para asegurar que el
            ".contains" funcionará correctamente*/
    ...
    public Usuario obtenerUsuario(String login, String passw) throws Exception {
        ...
        try {
            ...
            usuario.setDirecciones(direccionDAO.getDirecionesByUsuarioId(usuario.getId()));
