...
public class UsuarioDAO implements EntidadDAOBD<Usuario, Integer> {
    ...
    public Integer insert(Usuario usuario) throws SQLException {
        String insertUsuario = "insert into usuario(nombre, apellidos, sexo, fecha_nacimiento, dni, login, password, email"
                + ") values( ?,  ?,  ?,  ?,  ?,  ?,  ?,  ?)"; // no requiere id
        ...
        try {
            ...
            String insertDireccion = "insert into direccion_usuario(usuario_id, direccion, localidad,"
                    + "provincia, codigo_postal) values( ?,  ?,  ?,  ?,  ?)"; // no requiere id
    ...
    public Usuario getUsuarioByLoginPassw(String login, String passw) throws SQLException {
        String consulta = "select id, nombre, apellidos, sexo, fecha_nacimiento, dni, login, email, " + 
                "fecha_registro from usuario where login =  ? and  password =  ?";
