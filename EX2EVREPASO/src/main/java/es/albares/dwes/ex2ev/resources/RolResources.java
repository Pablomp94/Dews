...
public class RolResources {
    ...
    @GET
    @Path("/{codigo}/numUsuarios")
    //OBTENER EL NUMERO DE USUARIOS CON EL ROL CON PK (CODIGO)
    public Integer getNumUsuariosByRolId(@PathParam("codigo") String codigo){
        RolDAO rolDao = CDI.current().select(RolDAO.class).get();
        try {
            return rolDao.getNumUsuariosByRolId(codigo);
        } catch (SQLException ex) {
            Logger.getLogger(RolResources.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
